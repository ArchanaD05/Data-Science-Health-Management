...
The outliers seem to start from the cost value of 10,000
...
# Saving this data frame to duplicate data
datafile_backup <- datafile
```
...
datafile <- as.numeric(datafile, L = FALSE)
lmOut <- lm(cost~., data = datafile)
summary(lmOut)
...
datafile$cost <- as.factor(datafile$cost)
```
...
NB_Model1 <- naiveBayes(cost ~ ., data=NB_train, laplace=1)
NB_Model1
```

```{r}
#Evaluate the model
cost_pred1 <- predict(NB_Model1, NB_test, type="class")
cost_pred_table1 <- table(NB_test$cost, cost_pred1)
cost_pred_table1
```
```{r}
